...
3. 预测网络
    ...
    分割预测网络的卷积层，卷积核大小均为 3 × 3，反卷积层的卷积核大小为 2 × 2，步长 为 2，同时每个卷积层后面都有一个非线性激活层。

    Mask R-CNN 通过使用全卷积网络为每个预选框预测一个面积为m×m的 掩膜，这样可以保证掩膜中包含物体的位置分布信息。

    据分类分支得到的类别结果，选择分割分支对应的类别的输出，这样可以把分割与分类 两个任务解耦。